...
d2 = data |> 
     ...
     filter(n() == 2) |> 
     ungroup() |> 
     select(year,isocode,capital_per_capita,outstanding_loans_commercial_banks)
...
d2 = d2 %>%
     group_by(isocode) |> 
     summarise(across(.cols = c(capital_per_capita,outstanding_loans_commercial_banks),
                        .fns = function(x){
